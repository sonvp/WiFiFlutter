 ...
 * WifiIotPlugin
 */
public class WifiIotPlugin implements MethodCallHandler, EventChannel.StreamHandler, PluginRegistry.ActivityResultListener{
  private static final String TAG = "WifiIotPlugin";

  static final int REQUEST_CODE_SETTINGS = 99;
    ...
    do {
      if ((moWiFi.getConnectionInfo() != null &&
        ...
        if(connectedWifi(ssid)){
          ...
          moWiFi.enableNetwork(networkId, true);
...
//          moWiFi.reconnect();
    ...
    while (waiting_retries-- > 0 );
     ...
     resultsSettings = false;
    do {
      Log.d(TAG, "open settings");
      if (!connectedWifi(ssid) && !openedSettings && !resultsSettings) {
        openedSettings = true;
  ...
  public boolean connectedWifi(String ssid) {
  ...
  boolean resultsSettings = false;
  @Override
  public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
    switch (requestCode) {
      case REQUEST_CODE_SETTINGS:
        openedSettings=false;
        resultsSettings = true;
        break;
      default:
        return false;
    }
